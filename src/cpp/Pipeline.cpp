...
VkShaderModule Pipeline::createShaderModule(const std::vector<char>& code) {
	...
	// code.data() in the std::vector<char> will typically be aligned to at least
	// 4 bytes because modern memory allocators typically align allocations to a
	// word boundary (at least 4 bytes, and often more).
...
std::unique_ptr<PipelineConfigInfo> Pipeline::getDefaultPipelineConfigInfo(uint32_t width, uint32_t height) {
	std::unique_ptr<PipelineConfigInfo> configInfo = std::make_unique<PipelineConfigInfo>();
